/* mixture.c for R deSolve package
   ___________________________________________________

   Model File:  mixture.model

   Date:  Mon Apr 02 16:09:17 2018
...
#define ID_Cgas 0x000a
#define ID_Caerosol 0x000b

/* Parameters */
static double parms[46];
...
#define Vdot parms[39]
#define Qalv parms[40]
#define Kblood2air parms[41]
#define Fdeposited parms[42]
#define Fds parms[43]
#define DEalv parms[44]
#define Fgas parms[45]
...
/*----- Initializers */
void initmod_mix (void (* odeparms)(int *, double *))
{
  int N=46;
  odeparms(&N, parms);
}

void initforc_mix (void (* odeforcs)(int *, double *))
{
  int N=1;
  odeforcs(&N, forc);
}
...
void getParms_mix (double *inParms, double *out, int *nout) {
/*----- Model scaling */
...
void derivs_mix (int *neq, double *pdTime, double *y, double *ydot, double *yout, int *ip)
{
  ...
  yout[ID_Clungart] = ( Qcardiac * yout[ID_Cven] + Qalv * yout[ID_Cgas] ) / ( Qcardiac + Qalv / Kblood2air ) ;

  yout[ID_Cplasma] = y[ID_Aven] / Vven / Rblood2plasma ;

  yout[ID_Aplasma] = y[ID_Aven] / Rblood2plasma * ( 1 - hematocrit ) ;

  yout[ID_Cgas] = Cinh * Fgas ;

  yout[ID_Caerosol] = Cinh * ( 1 - Fgas ) ;

  ydot[ID_Agutlumen] = - kgutabs * y[ID_Agutlumen] + Vdot * Fdeposited * yout[ID_Caerosol] ;
  ...
  ydot[ID_Aart] = Qcardiac * ( yout[ID_Clungart] - yout[ID_Cart] ) + Vdot * yout[ID_Caerosol] * DEalv ;
...
void jac_mix (int *neq, double *t, double *y, int *ml, int *mu, double *pd, int *nrowpd, double *yout, int *ip)
{
...
/*----- Events calculations: */
void event_mix (int *n, double *t, double *y)
{

} /* event */

/*----- Roots calculations: */
void root_mix (int *neq, double *t, double *y, int *ng, double *gout, double *out, int *ip)
{
