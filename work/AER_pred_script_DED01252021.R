...
#Make fraction unbound correction table from training data
FUcorrectiontab=aggregate(Fu_assay~Bin, data=train,FUN="median")
...
human.httk.data.complete[human.httk.data.complete$Human.Clint.Bin3_Names == j,"Human.Clint.Bin3.MD"] <- median(subset(train, Bin.Adj.Bin3 == j)$Clearance)
human.httk.data.complete[human.httk.data.complete$Human.Clint.Bin3_Names == j,"Human.Clint.Bin3.SD"] <- sd(subset(train,Bin.Adj.Bin3 == j)$Clearance)
}
...
human.httk.data.complete[human.httk.data.complete$Human.Clint.QSAR.Bin3_Names == j,"Human.Clint.QSAR.Bin3.MD"] <- median(subset(train,Bin.Adj.Bin3 == j)$Clearance)
human.httk.data.complete[human.httk.data.complete$Human.Clint.QSAR.Bin3_Names == j,"Human.Clint.QSAR.Bin3.SD"] <- sd(subset(train,Bin.Adj.Bin3 == j)$Clearance)
}
...
Comp1=c("Clearance.Adj", "Human.Funbound.plasma", "CSS.InVitro")
...
#Remove chemicals not in Fub AD
FupAD=read.csv("Fup/Full_fup_predictionsAll_40desc_060320_AD.csv")
FupADout=FupAD[FupAD$AD_out==1, "CAS"]
human.hits[which(human.hits$CAS%in%FupADout),"CAS"]
#Chemicals in ToxCast test set outside AD
"10265-92-6" 
"56-95-1"

human.hits=human.hits[-c(which(human.hits$CAS%in%FupADout)),]
####
...
#Exceeds
dim(human.hits1[human.hits1$AER_IV.CLBin3.MD<1,])[1] #15 exceeds for in vitro
dim(human.hits1[human.hits1$AER_QSAR_FubPred_CLPredBin3.MD<1,])[1] #26 exceeds for QSAR


#FN
#QSAR
FNQ=which(human.hits1[human.hits1$AER_IV.CLBin3.MD<1, "CAS"]%in%human.hits1[human.hits1$AER_QSAR_FubPred_CLPredBin3.MD<1, "CAS"]==FALSE)
#RandClit
FNCLrand=which(human.hits1[human.hits1$AER_IV.CLBin3.MD<1, "CAS"]%in%human.hits1[human.hits1$AER_FubPred_CLYrand<1, "CAS"]==FALSE)
#RandFub
FNFubrand=which(human.hits1[human.hits1$AER_IV.CLBin3.MD<1, "CAS"]%in%human.hits1[human.hits1$AER_FubYrand_CLPredBin3<1, "CAS"]==FALSE)
#RandBoth 
FNFubrandClrand=which(human.hits1[human.hits1$AER_IV.CLBin3.MD<1, "CAS"]%in%human.hits1[human.hits1$AER_FubYrand_CLYrand<1, "CAS"]==FALSE)

#YrandFub,and both 
FNR=FNFubrand/(length(human.hits1[human.hits1$AER_IV.CLBin3.MD< 1, "CAS"]) + FNFubrand)
FNR=FNCLrand/(length(human.hits1[human.hits1$AER_IV.CLBin3.MD< 1, "CAS"]) + FNCLrand)

#QSAR
FP=length(which(human.hits1[human.hits1$AER_QSAR_FubPred_CLPredBin3.MD<1, "CAS"]%in%human.hits1[human.hits1$AER_IV.CLBin3.MD<1, "CAS"]==FALSE))
FPR=FP/(length(human.hits1[human.hits1$AER_IV.CLBin3.MD>=1, "CAS"]) + FP) 

#Yrandfub
FP=length(which(human.hits1[human.hits1$AER_FubYrand_CLPredBin3<1, "CAS"]%in%human.hits1[human.hits1$AER_IV.CLBin3.MD<1, "CAS"]==FALSE))
FPR=FP/(length(human.hits1[human.hits1$AER_IV.CLBin3.MD>=1, "CAS"]) + FP) 

#Yrandclint
FP=length(which(human.hits1[human.hits1$AER_FubPred_CLYrand<1, "CAS"]%in%human.hits1[human.hits1$AER_IV.CLBin3.MD<1, "CAS"]==FALSE))
FPR=FP/(length(human.hits1[human.hits1$AER_IV.CLBin3.MD>=1, "CAS"]) + FP) 

#Yrandboth
FP=length(which(human.hits1[human.hits1$AER_FubYrand_CLYrand<1, "CAS"]%in%human.hits1[human.hits1$AER_IV.CLBin3.MD<1, "CAS"]==FALSE))
FPR=FP/(length(human.hits1[human.hits1$AER_IV.CLBin3.MD>=1, "CAS"]) + FP) 
...
Fig <- Fig + scale_x_continuous(breaks=seq(0,90,10), limits=c(0,95))
  ...
  if (sourcelabel==TRUE){
    Fig <- Fig + annotate(geom="Text", x=1, y=max(log(human.hits$AER_IV))*0.9, label=SourceLabel, color="black", size=8, hjust=0)
  }
...
SuppTab=subset(human.hits, select=c("Compound", "CAS", "DTXSID", "Human.Clint.Correct", "Human.Clint.Bin3", "Human.Funbound.plasma",
...
write.csv(Chemicallist, file=paste("AER/ToxCast_AER_ChemicalSet_", writesuff,".csv", sep = ""))
...
###Note: this is the loss of the ToxCast set at each set:
Number Chemicals:
  #Start   	                      99	
  #ToxCastAD	                    97	
  #Through CSS	                  97	
  #Merge with Tox21.acc.numeric	  92	
  #Merge with Fup21	              90
