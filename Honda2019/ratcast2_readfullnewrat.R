...
read_new_rat <- function(){
  ...
  Clint1 <- Clint.Raw[,list(aliquot=Test.Article,Concentration,microM, Clearance = Clearance..?l.min.million.cells.)] %>% 
  ...
  new.rat[,notes:="none"] %>% 
    .[casrn %in% repeat_cas, notes:="repeat"] %>% 
    .[casrn %in% old.rat$CAS, notes:="replicate_chem_meas_v1.8"] %>% 
    .[,c("use_fup","use_clint") := FALSE] %>% 
    .[notes != "repeat" & !is.na(Funbound.plasma),use_fup:=TRUE] %>%
    .[notes == "none" & !is.na(clearance),use_clint := TRUE] %>% 
    .[casrn=="81-81-2" & Fub100 != 0.005, c("use_fup","use_clint") := TRUE]
  
  full.new.rat <- copy(new.rat)
  save(full.new.rat,file=paste0(loc.wd,"/NewData/full_new_rat_04Dec2018.RData"))
  
  partial.new.rat <- copy(new.rat)[notes == "none" & (use_fup | use_clint) & !(use_fup & use_clint),]
  
  save(partial.new.rat,file= paste0(loc.wd,"/NewData/partial_new_rat_04Dec2018.RData"))
