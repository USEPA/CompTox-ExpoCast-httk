...
Health.Canada <- read.xls("Summary HLM Chem list Apr 2019.xlsx",stringsAsFactors=F)
Health.Canada <- subset(Health.Canada,HLM.type=="cellular"&Non.detect=="")
Health.Canada <- Health.Canada$DTXSID
JRC <- read.xls("APCRA_HTTK_JRC list_77 chemicals_2018.xlsx",stringsAsFactors=F,sheet=3)$DTXSID
APCRA <- read.csv("APCRA-prospective-09042019.csv",stringsAsFactors=F)$DTXSID
...
all.chems <- sort(unique(c(EFSA.interest,APCRA,Cyprotex.forgot, Cyprotex.Caco2,Health.Canada,JRC,unilever, invivo,Cyprotex.attempted,httk110.human,httk110.rat,shark1,shark2,shark3,sharkbackfill,TSCA.430,TSCA.poc)))
...
for (this.id in all.chems)
{
      ...
      this.row <- cbind(this.row,t(as.data.frame(rep(0,times=18))))
      colnames(this.row) <- c("DTXSID","TSCA","TSCA.PoC","TSCA.430","Tox21","ToxCast","APCRA","Unilever","Health.Canada","JRC","EFSA.Interest","HTTK.1.10.Human","HTTK.1.10.Rat","HTTK.Cyprotex.Clint","HTTK.Cyprotex.Fup","HTTK.Cyprotex.Caco2","HTTK.Cyprotex.Failed","HTTK.Cyprotex.Forgot","Shark.Tank")
      ...
      if (this.id %in% APCRA) this.row["APCRA"] <- 1
      if (this.id %in% JRC) this.row["JRC"] <- 1
      ...
      if (this.id %in% Cyprotex.Caco2) this.row["HTTK.Cyprotex.Caco2"] <- 1
      if (this.id %in% Cyprotex.failed) this.row["HTTK.Cyprotex.Failed"] <- 1
      if (this.id %in% Cyprotex.forgot) this.row["HTTK.Cyprotex.Forgot"] <- 1
      if (this.id %in% Tox21) this.row["Tox21"] <- 1
      if (this.id %in% ToxCast) this.row["ToxCast"] <- 1
      if (this.id %in% unilever) this.row["Unilever"] <- 1
      if (this.id %in% c(shark1,shark2,shark3,sharkbackfill)) this.row["Shark.Tank"] <- 1
