...
rm(list=ls()) 
setwd("C:\\CompTox-ExpoCast-httk\\datatables")
...
#
#
# Data added by ICF 9/14/23:

# Black 2021 Data
black.test_.1 <- as.data.frame(readxl::read_xlsx(
  path = "httk upload 1.xlsx",
  sheet = "Black 2021 starting conc 0.1uM"))
black.test_.1 <- black.test_.1 %>% filter(!is.na(Clint))
chem.physical_and_invitro.data <- add_chemtable(black.test_.1, 
                                     current.table=chem.physical_and_invitro.data, 
                                     data.list = list(Compound = 'Compound',
                                                      CAS = 'CASRN', 
                                                      DTXSID = 'DTXSID',
                                                      ...
                                                      Clint = 'Clint', 
                                                      Funbound.plasma = 'Fup'),
                                     reference = 'Black 2021', overwrite = FALSE)
chem.physical_and_invitro.data <- check_duplicates(
  chem.physical_and_invitro.data, check.cols="Compound")

black.test_1 <- as.data.frame(readxl::read_xlsx(
  path = "httk upload 1.xlsx",
  sheet = "Black 2021 starting conc 1uM"))
chem.physical_and_invitro.data <- add_chemtable(black.test_1, 
                                    current.table=chem.physical_and_invitro.data, 
                                    data.list = list(Compound = 'Compound',
                                                     CAS = 'CASRN', 
                                                     DTXSID = 'DTXSID',
...
williamson.test <- as.data.frame(readxl::read_xlsx(
  path = "httk upload 1.xlsx",
  sheet = "Williamson 2020"))
chem.physical_and_invitro.data <- add_chemtable(williamson.test, 
                                       current.table=chem.physical_and_invitro.data,
                                       data.list = list(Compound = 'Compound',
                                                        CAS = 'CASRN', 
                                                        DTXSID = 'DTXSID',
                                                        ...
                                                        Clint = 'Clint', 
                                                        Funbound.plasma = 'Funbound.plasma'),
                                       reference = 'Williamson 2020', overwrite = FALSE)
chem.physical_and_invitro.data <- check_duplicates(
  chem.physical_and_invitro.data, check.cols="Compound")

# Yamagata 2017 Data
yamagata.test <- as.data.frame(readxl::read_xlsx(
  path = "httk upload 1.xlsx",
  sheet = "Yamagata 2017"))
yamagata.test_first <- yamagata.test %>% filter(pKa.reference == 'Poulin & Haddad, 2013')
yamagata.test_second <- yamagata.test %>% filter(pKa.reference == 'Yamagata 2017', CASRN != '#NA')
chem.physical_and_invitro.data <- add_chemtable(yamagata.test_first, 
                                           current.table=chem.physical_and_invitro.data,
                                           data.list = list(Compound = 'Compound',
                                                            CAS = 'CASRN', 
                                                            DTXSID = 'DTXSID',
...
chem.physical_and_invitro.data <- add_chemtable(yamagata.test_second, 
                                            current.table=chem.physical_and_invitro.data,
                                            data.list = list(Compound = 'Compound',
                                                             CAS = 'CASRN',
                                                             DTXSID = 'DTXSID', 
...
# Zanelli 2019 Data
zanelli.2019.data <- as.data.frame(readxl::read_xlsx(
  path = "httk upload 1.xlsx",
  sheet = "Zanelli 2019 ISTD MSC1815677"))
chem.physical_and_invitro.data <- add_chemtable(zanelli.2019.data, 
                                         current.table=chem.physical_and_invitro.data, 
                                         data.list = list(Compound = 'Compound',
                                                          CAS = 'CASRN', 
                                                          Species = 'Species', 
