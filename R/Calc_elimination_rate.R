...
calc_elimination_rate <- function(chem.cas=NULL,chem.name=NULL,parameters=NULL,species="Human",suppress.messages=F,
                                 default.to.human=F,restrictive.clearance=T,Funbound.plasma.pc.correction=T,well.stirred.correction=T)
{
    ...
    if(is.null(parameters)){
      ...
      Vd <- calc_vdist(chem.cas=chem.cas,chem.name=chem.name,species=species,suppress.messages=T,default.to.human=default.to.human,Funbound.plasma.pc.correction=Funbound.plasma.pc.correction) 
    }else{ 
