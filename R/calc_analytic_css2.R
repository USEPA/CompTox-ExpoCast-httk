...
calc_analytic_css2 <- function(chem.name=NULL,
                               chem.cas = NULL,
                               parameters=NULL,
                               daily.dose=1,
                               output.units='uM',
...
# Retrieve the molecular weight if the parameters argument isn't given:'
  ...
  {
    Css <-eval(model.list[[model]]$analytic.css.func,
      chem.name=chem.name,
      chem.cas = chem.cas,
...
model.list[["3compss"]]$analytic.css.func <- "calc_analytic_css_3compss"
...
# Scale hourly dose rate by fraction absorbed and bioavailability due to first
# pass metabolism:
  hourly.dose <- hourly.dose * parameters$Fgutabs * parameters$hepatic.bioavailability
...
model.list[["1comp"]]$analytic.css.func <- "calc_analytic_css_1comp"
...
model.list[["3comp"]]$analytic.css.func <- "calc_analytic_css_3comp"
