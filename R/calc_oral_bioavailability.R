...
calc_fbio.oral <- function(Params = NULL,
  ...
  if(is.null(Params)){
    ...
    Params <- parameterize_fbio(chem.cas = chem.cas,
                                     chem.name = chem.name,
                                     species = species,
  ...
  fhep.oral <- calc_hepatic.bioavailability(Params = Params)
...
calc_fabs.oral <- function(Params = NULL,
  ...
  # Required parameters
  req.param <- c("Qtotal.liverc", "BW", "cl_us", "Rblood2plasma", "Caco2.Pab")
...
calc_hepatic.bioavailability <- function(Params = NULL,
                           chem.cas = NULL,
                           chem.name = NULL,
                           species = "Human",
                           clint.pvalue.threshold=0.05,
  ...
  # Required parameters
  req.param <- c("Qtotal.liverc", "BW", "cl_us", "Rblood2plasma")
  ...
  { 
    Params[['hepatic.bioavailability']] <- 
      Qliver / (Qliver + Params$Funbound.plasma * Params$cl_us / Params$Rblood2plasma) # Units cancel (i.e., unitless)
  } else {
    Params[['hepatic.bioavailability']] <- 
      Qliver / (Qliver + Params$cl_us / Params$Rblood2plasma) # Units cancel (i.e., unitless)
  }
  
  return(Params[['hepatic.bioavailability']])
}
  
# Parameterize bioavailability functions. Should match parameterize_steady_state.
parameterize_fbio <- function(chem.cas=NULL,
                              chem.name=NULL,
                              species="Human",
                              clint.pvalue.threshold=0.05,
                              default.to.human=F,
                              ...
                              Caco2.Pab.default = 1)
{
  ...
  Params[["Caco2.Pab"]] <- Caco2.Pab.point # log10( 10 ^ -6 cm/s)
  ...
  # Calculate and return unscaled clearance
  Params[["cl_us"]] <- calc_hepatic_clearance(parameters=Params,
