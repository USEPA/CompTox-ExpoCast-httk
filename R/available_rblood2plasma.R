available_rblood2plasma <- function(chem.cas=NULL,chem.name=NULL,species='Human',Funbound.plasma.pc.correction=T)
{
  ...
  if(is.na(Rblood2plasma)){
    ...
    if(chem.cas %in% get_cheminfo(species=species,model='schmitt')){
      Rblood2plasma <- calc_rblood2plasma(chem.cas=chem.cas,species=species,Funbound.plasma.pc.correction=Funbound.plasma.pc.correction)
      ...
      Rblood2plasma <- calc_rblood2plasma(chem.cas=chem.cas,species=species,default.to.human=T,Funbound.plasma.pc.correction=Funbound.plasma.pc.correction)
