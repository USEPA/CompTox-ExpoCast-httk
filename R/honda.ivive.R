honda.ivive <- function(method="Honda1",tissue="liver")
{
  ...
  restrictive.clearance<-bioactive.free.invivo<-statistic.used<-NULL
...
#    tk.statistic.used <<- "mean"
    warning("Argument method = \"Honda1\" uses plasma concentration, restrictive clearance, and treats the unbound concentration as bioactive.\n For IVIVE, the input conc should be the free in vitro bioactivity concentration calculated using 
              armitage_eval.")
