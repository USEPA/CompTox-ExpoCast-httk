solve_gas_pbtk_Vdot_optimization <- function(chem.name = NULL,
                    chem.cas = NULL,
                    times=NULL,
                    parameters=NULL,
                    days=10,
                    ...
                    ...)
{
  ...
  if(is.null(parameters)){
    parameters <- parameterize_gas_pbtk_Vdot_optimization(chem.cas=chem.cas,chem.name=chem.name,species=species,default.to.human=default.to.human,suppress.messages=suppress.messages,
                                    adjusted.Funbound.plasma=adjusted.Funbound.plasma,regression=regression, vmax.km=vmax.km, vmax = vmax, km = km, vp.koa = vp.koa, vp = vp, koa = koa, muc = muc, lP = lP, Vdot = Vdot)                                  
  }else{
  ...
  if(vmax.km){
    parameters <- initParms_inh_mm(parameters[!(names(parameters) %in% c("Fhep.assay.correction","Krbc2pu","million.cells.per.gliver","Fgutabs","Funbound.plasma"))])
    state <-initStates_inh_mm(parameters,state) 
    ...
    Forc <- list(forcing(air.concentration, period, 0,exposure, times, vmax, km))  # possible to have more than one in list
    if(!is.null(dosing.matrix)) Forc <- dosing.matrix
    out <- ode(y = state, times = times, func="derivs_inh_mm", parms = parameters,method=method,rtol=rtol,atol=atol, dllname="httk",initfunc="initmod_inh_mm", nout=length(Outputs_inh_mm),outnames=Outputs_inh_mm,initforc= "initforc_inh_mm",forcing=Forc,fcontrol=list(method='constant',rule=2,f=0),...)   
  }else if(diffeq){
  parameters <- initParms_inh_diffeq(parameters[!(names(parameters) %in% c("Fhep.assay.correction","Krbc2pu","million.cells.per.gliver","Fgutabs","Funbound.plasma"))])
  state <-initStates_inh_diffeq(parameters,state) 
  ...
  if(!is.null(dosing.matrix)) Forc <- dosing.matrix
  out <- ode(y = state, times = times, func="derivs_inh_diffeq", parms = parameters,method=method,rtol=rtol,atol=atol, dllname="httk",initfunc="initmod_inh_diffeq", nout=length(Outputs_inh_diffeq),outnames=Outputs_inh_diffeq,initforc= "initforc_inh_diffeq",forcing=Forc,fcontrol=list(method='constant',rule=2,f=0),...)   
  }else if(muc){
  parameters <- initParms_inh_muc(parameters[!(names(parameters) %in% c("Fhep.assay.correction","Krbc2pu","million.cells.per.gliver","Fgutabs","Funbound.plasma"))])
  state <-initStates_inh_muc(parameters,state) 
  ...
  Forc <- list(forcing(air.concentration, period, 0,exposure, times))  # possible to have more than one in list
  if(!is.null(dosing.matrix)) Forc <- dosing.matrix
  out <- ode(y = state, times = times, func="derivs_inh_muc", parms = parameters,method=method,rtol=rtol,atol=atol, dllname="httk",initfunc="initmod_inh_muc", nout=length(Outputs_inh_muc),outnames=Outputs_inh_muc,initforc= "initforc_inh_muc",forcing=Forc,fcontrol=list(method='constant',rule=2,f=0),...)   
  }else{
