...
# In the table we create each word in most column names is capitalized:
  exceptions <- c("Clint.pValue","logP","logMA","logPwa","MW","CAS","CAS.Checksum","pKa_Donor","pKa_Accept","SMILES.desalt","DSSTox.GSID", "HL")
  ...
  chem.phys.cols<-c("MW","logP","logPwa","pKa_Donor","pKa_Accept","logMA", "HL")
...
add_chemtable_devel <- function(new.table, data.list, current.table=NULL, reference=NULL,
                          species=NULL, overwrite=F)
{
# Let's make the capitalization consistent in data.list:
  exceptions <- c("Clint.pValue","logP","logPwa","logMA","MW","CAS","CAS.Checksum","pKa_Donor","pKa_Accept","SMILES.desalt","DSSTox.GSID", "HL")
