...
#' @keywords Solve
#' @export solve_aerosol_pbtk
solve_aerosol_pbtk <- function(chem.name = NULL,
                    chem.cas = NULL,
                    times=NULL,
                    parameters=NULL,
                    days=10,
                    ...
                    ...)
{
  ...
  parameters[['Fraction_unbound_plasma']] <- parameters[['Funbound.plasma']]
    ...
    out <- ode(y = state, times = times, func="derivs_aerosol", parms = parameters,method=method,rtol=rtol,atol=atol, dllname="httk",initfunc="initmod_aerosol", nout=length(Outputs_aerosol),outnames=Outputs_aerosol,initforc= "initforc_aerosol",forcing=Forc,fcontrol=list(method='constant',rule=2,f=0),...) 
