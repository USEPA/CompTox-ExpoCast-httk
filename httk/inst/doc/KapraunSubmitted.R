...
## ----chem_numbers, eval=TRUE--------------------------------------------------
length(get_cheminfo(model="fetal_pbtk"))

## ----load_aylward_data, eval = TRUE-------------------------------------------
...
## ----process_aylward_data, eval = TRUE----------------------------------------
...
## ----cleanup_repeated_aylward_figure, eval = TRUE-----------------------------
MFdata.main <- NULL
...
## ----aylward_figure_no_fup, eval = TRUE---------------------------------------
...
## ----aylward_analysis_text, eval = TRUE---------------------------------------
...
## ----aylward_figure_with_fup, eval = TRUE-------------------------------------
...
## ----curley1969_makepreds, eval = TRUE----------------------------------------
...
## ----curley1969_figure_compounds, eval = TRUE---------------------------------
...
## ----curley1969_figure_tissues, eval = TRUE-----------------------------------
...
## ----read_pksim_pcs, eval = FALSE---------------------------------------------
#  pksim.pcs <- as.data.frame(read_excel("PartitionCoefficients.xlsx"))
#  dim(pksim.pcs)
#  for (this.id in unique(pksim.pcs$DTXSID))
#  {
#    httk.PCs <- suppressWarnings(predict_partitioning_schmitt(dtxsid=this.id,
#                                             suppress.messages = TRUE))
#    p <-
#      suppressWarnings(parameterize_fetal_pbtk(dtxsid=this.id,
#      suppress.messages = TRUE))
...
## ----brain_uncertainty_prop_table, eval = TRUE--------------------------------
...
for (this.cas in maternal.list)
{
  ...
  fup <- PC.table[PC.table$CAS==this.cas,"fup"]
...
## ----Rfetbrain_uncertainty, eval = TRUE---------------------------------------
...
## ----Wang_Uncertainty_Propagation, eval = TRUE--------------------------------
...
## ----MFratio_allhttk_preds, eval = FALSE--------------------------------------
#  times <- sort(unique(c(seq(13 * 7, 40 * 7, 0.25),seq(278,280,.1))))
#  
#  MFratio.pred <- NULL
#  all.chems <- get_cheminfo(model="fetal_pbtk", info=c("Compound","DTXSID","CAS","Funbound.plasma"))
...
## ----MFratio_allhttk_figure, eval = FALSE-------------------------------------
...
## ----MFratio_allhttk_stats, eval = FALSE--------------------------------------
