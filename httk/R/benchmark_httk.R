...
#'
#' @param in_vivo_css.check A character string of three numbers separated by two 
#' dashes (defaults to TRUE)
#'
                           ...
                           make.plots=TRUE)
{
  ...
  if (basic.check)
  {
    ...
    set.seed(1234)
    mc1 <- calc_mc_css(chem.cas="80-05-7",
                       output.units="mg/L",
                       suppress.messages=suppress.messages)[1]
    set.seed(1234)
    mc2 <- calc_mc_css(chem.cas="80-05-7",
                       output.units="uM",
                       suppress.messages=suppress.messages)[1]
  ...
  if (in_vivo_css.check)
  {
    ...
    if ("parameterize.args" %in% formalArgs(calc_analytic_css))
    {
      FitData$Css.pred <- sapply(FitData$CAS,
        function(x) as.numeric(try(calc_analytic_css(
                chem.cas=x,
                species="Rat",
                model="3compartmentss",
                suppress.messages=suppress.messages,
    ...
    benchmarks[["in_vivo_css"]] <- list(RMSLE.InVivoCss = RMSLE.invivocss,
