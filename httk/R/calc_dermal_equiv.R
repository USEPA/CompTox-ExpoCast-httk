...
calc_dermal_equiv <- function(conc,chem.name=NULL,chem.cas=NULL,parameters=NULL,days=20,doses.per.day=3,depth=0.3,skin.pH=7,Vmedia=0.001,Fskinexposed=0.1,...){
  if(is.null(parameters)){
    parameters <- parameterize_dermal_pbtk(chem.name=chem.name,chem.cas=chem.cas,depth=depth,skin.pH=skin.pH)
    parameters$Vmedia <- Vmedia
    parameters$Fskinexposed <- Fskinexposed
  }
  out <- solve_dermal_pbtk(chem.name=chem.name,chem.cas=chem.cas,days=days,parameters=parameters,doses.per.day=doses.per.day,...,suppress.messages=T)
