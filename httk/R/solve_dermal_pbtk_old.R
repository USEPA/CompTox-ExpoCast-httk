                    ...
                    vmax.km=F,...)
{
  ...
  if(is.null(parameters)){
    parameters <- parameterize_dermal_pbtk(chem.cas=chem.cas,chem.name=chem.name,species=species,default.to.human=default.to.human,suppress.messages=suppress.messages,
                                    adjusted.Funbound.plasma=adjusted.Funbound.plasma,regression=regression,skin_depth=skin_depth,skin.pH=skin.pH,vmax.km=vmax.km) 
  }else{
  ...
  if(parameters$Vskinc * parameters$BW - parameters$totalSA * parameters$skin_depth * 0.001 * parameters$Fskinexposed < 0) stop('Exposed volume of skin is larger than skin volume.')
  }
...
parameters[["MW"]] <- NULL
  ...
  Vskinexposed <- parameters$Fskinexposed * parameters$totalSA * parameters$skin_depth * 0.001  
