...
human.invivo.hl.fig <- ggplot(data=human.invivo.hl) +
  ...
  scale_y_log10(label=scientific_10)+
  theme_bw()  +
  annotate("text", x = 5, y = 10^4, hjust=0,
...
for (this.chem in pfas.httk.table$DTXSID)
{
  ...
  fractions <- try(calc_clearance_frac(dtxsid=this.chem, model="sumclearancespfas"))
  if (!is(fractions, "try-error"))
  {
    ...
    pfas.httk.table[this.row, "Frac.Exhalationr"] <- fractions[["Qalvc"]]
  }
}
...
Fig.Cssa <- ggplot(data=pfas.httk.table)+
  geom_histogram(aes(Css.Exhale.Foldchange))
Fig.Cssb <- ggplot(data=pfas.httk.table)+
  geom_histogram(aes(Css.PFAS.Foldchange))
