...
conc_data$ACTUAL_ORIGINAL_CONC_U <- conc_data$ORIG_CONC_U
PPMVCONVERT <- 1
```
...
gas.units <- unique(subset(conc_data,
  SAMPLING_MATRIX %in% gas.media)$ORIG_CONC_U)
target.unit <- "ppmv"
...
tissue.units <- unique(subset(conc_data,
  SAMPLING_MATRIX %in% tissue.media)$ORIG_CONC_U)
target.unit <- "uM"
...
```{r Standardize Units}
these.chems <- unique(subset(conc_data,ORIG_CONC_U=="ppmv")$DTXSID)
for (this.chem in these.chems)
{
  # Use HTTK unit conversion:
  this.factor <- convert_units(
    input.units="ppmv", output.units="um", dtxsid=this.chem)
      
  # Scale the observation    
  conc_data[conc_data$DTXSID==this.chem & 
              conc_data$ORIG_CONC_U=="ppmv","CONCENTRATION"] <-
    this.factor * conc_data[
      conc_data$DTXSID==this.chem & 
        conc_data$ORIG_CONC_U=="ppmv","CONCENTRATION"]
  # Change the reported unit
  conc_data[conc_data$DTXSID==this.chem & 
              conc_data$ORIG_CONC_U=="ppmv","ORIG_CONC_U"] <-
    "uM"
}
```
