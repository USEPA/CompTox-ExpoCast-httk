...
Normalize the units of the concentration data:
```{r data}
for (this.unit in unique(conc_data[,"ORIG_CONC_U"]))
{
  print(this.unit)
  these.chems <- unique(subset(conc_data,ORIG_CONC_U==this.unit)$DTXSID)
  for (this.chem in these.chems)
  {
    # convert liquid concentrations to uM
    if (this.unit %in% c(
      "ug/mL","mg/L","ug/L","nmol/L","ug/dL", "percent", "umol/L"))
      target.unit <- "uM"
    # convert gas concentrations to ppmv
    else if (this.unit %in% c(
      "ug/g")) target.units <- "ppmv"
    ...
    conc_data[conc_data$DTXSID==this.chem & 
      conc_data$ORIG_CONC_U==this.unit,"CONCENTRATION"] <-
      this.factor * conc_data[
        conc_data$DTXSID==this.chem & 
        conc_data$ORIG_CONC_U==this.unit,"CONCENTRATION"]
    # Change the reported unit
    conc_data[conc_data$DTXSID==this.chem & 
      conc_data$ORIG_CONC_U==this.unit,"ORIG_CONC_U"] <-
      target.unit
  }
}
...
# Make sure we get conc's at the observed times:
    ...
    {
      if (unique_scenarios[i,"ORIG_CONC_U"] == "ppmv")
      {
        solve$simconc <- solve$Cendexhppmv
        solve$unit <- "ppmv"
      } else if (unique_scenarios[i,"ORIG_CONC_U"] == "uM") {
        solve$simconc <- solve$Cendexh
        solve$unit <- "uM"
      } else if (unique_scenarios[i,"ORIG_CONC_U"] == "ug/L") {
        solve$simconc <- solve$Cendexh
        solve$unit <- "uM"
      }
...
Identify the appropriate matrix (for example, exhaled breath) for each observation:
```{r match_matrix}
...
#          simobscomb$simconc[j] <- simobscomb$Cendexhppmv[j] 
...
The observations in simobsfullare in both uM and ppmv -- standardize to uM
```{r Standardize Units}
convert_units(input.units="ppmv", output.units="uM")
