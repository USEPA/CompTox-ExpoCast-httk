...
#individual
armitageOutput_default.dt[,rmsle_ccell:=rmsle(Reported_Ccell_µM, ccells), by = Name]

#total
rmsle(armitageOutput_default.dt$Reported_Ccell_µM, armitageOutput_default.dt$ccells)
#0.5819282

#curated values

#individual
armitageOutput_curated.data.dt[,rmsle_ccell:=rmsle(Reported_Ccell_µM, ccells), by = Name]

#total
rmsle(armitageOutput_curated.data.dt$Reported_Ccell_µM, armitageOutput_curated.data.dt$ccells)
#0.5659809

library(ggrepel)

basicvalues_httk_plot<-ggplot(armitageOutput_default.dt, aes(Reported_Ccell_µM, ccells, color = Name, label = round(rmsle_ccell, 2)))+
  geom_point(size = 4) + 
  geom_abline(intercept=0,slope=1, linetype = "dashed") +
  geom_text_repel() +
                        scale_y_continuous(labels = function(x) sprintf("%g", x),  limits = c(1, 10000), trans='log10') +
                        scale_x_continuous(labels = function(x) sprintf("%g", x),limits = c(1, 10000),  trans='log10') +
                        xlab("Reported Ccell (\U00B5M)") + 
                        ylab("Httk Predicted CCell (\U00B5M)")+
                        ggtitle("Httk using Default Physchem Input")+
...
curatedvalues_httk_plot<-ggplot(armitageOutput_curated.data.dt, aes(Reported_Ccell_µM, ccells, color = Name, label = round(rmsle_ccell, 2)))+
  geom_point(size = 4) + 
  geom_abline(intercept=0,slope=1, linetype = "dashed") +
  geom_text_repel() +
                        scale_y_continuous(labels = function(x) sprintf("%g", x),  limits = c(1, 10000), trans='log10') +
                        ...
                        xlab("Reported Ccell (\U00B5M)") + 
                        ylab("Httk Predicted CCell (\U00B5M)")+
                        ggtitle("Httk using Curated Physchem Input")+
                        theme_bw() + #now make it look good
                        theme(text=element_text(size=15),
                              legend.position="bottom",
                              panel.border = element_blank(), 
...
basicvalues_httk_plot
curatedvalues_httk_plot
...
#append to table
options.table<-rbind(options.table, c(round(option.basic_RMSLE ,2), "All Default"))
...
#append to table
options.table<-rbind(options.table, c(round(option.kbsa_RMSLE, 2), "option.kbsa=TRUE"))
...
#append to table
options.table<-rbind(options.table, c(round(option.swat_RMSLE, 2), "option.swat2=TRUE"))
...
#append to table
options.table<-rbind(options.table, c(round(option.kpl_RMSLE, 2), "option.kpl2=TRUE"))
...
#append to table
options.table<-rbind(options.table, c(round(option.plast_RMSLE, 2), "option.plastic=FALSE"))
...
#append to table
options.table<-rbind(options.table, c(round(option.bottom_RMSLE,2), "option.bottom=FALSE"))
...
#append to table
options.table<-rbind(options.table, c(round(option.restion_RMSLE,2), "restrict.ion.partitioning = TRUE"))
