...
load("C:/Users/mscherer/httk-dev/httk/data/ArmitagePlus_Vignette_26MAY25.RData")
...
parameters_only[, timepoint_hr := ExposureDuration_hrs] %>% 
  ...
  .[, cell_type := CellType] %>% 
  .[, assay_name := gsub(", ", "", parameters_only$Citation)]
...
parameters_only[, assay_component_endpoint_name := paste("IVD", gsub(", ", "", assay_name), sep="")]

#tack on the "interesting" part for each assay
parameters_only[assay_component_endpoint_name=="IVDBellwon2015b" | 
                  assay_component_endpoint_name == "IVDBroeders2013" |
                  assay_component_endpoint_name == "IVDWei2010",
                assay_component_endpoint_name := paste(assay_component_endpoint_name, cell_type, sep="_")]

parameters_only[assay_component_endpoint_name=="IVDMundy2004", 
                assay_component_endpoint_name := paste(paste(assay_component_endpoint_name, paste(timepoint_hr, "hr", sep = ""), sep="_"), paste(FBSf, "FBSf", sep = ""), sep = "_")]

#bind and reassign over the invitro.assay.params table so it passes to sarea calculation function
invitro.assay.params<-rbind(invitro.assay.params, parameters_only)
# should have 30 + 1649 = 1679 rows

#add the assay_component_endpoint_name to the experimental dataframe 
IVD_Manuscript_data[, assay_component_endpoint_name := paste("IVD", gsub(", ", "", Citation), sep="")]
...
IVD_Manuscript_data[assay_component_endpoint_name=="IVDMundy2004", 
                assay_component_endpoint_name := paste(paste(assay_component_endpoint_name, 
                                                             paste(ExposureDuration_hrs, "hr", sep = ""), sep="_"), 
                                                       paste(FBSf, "FBSf", sep = ""), sep = "_")]

all(unique(IVD_Manuscript_data[,.(assay_component_endpoint_name)]) %in% invitro.assay.params[,.(assay_component_endpoint_name)])



invitro.assay.params$assay_component_endpoint_name
class(invitro.assay.params)
```
...
combodata %>% dplyr::filter(Citation.x == "Broeders, 2013") %>% select(nomconc)
...
combodata[(ccells>concentration_cells), length(unique(ChemicalName.x))] #38 chemicals where armitage ccells is larger
combodata[(ccells<concentration_cells), length(unique(ChemicalName.x))]   #16 chemicals where armitage is smaller than Kramer
...
# basicvalues_httk_plot<-ggplot(armitageOutput_default.dt, aes(ccells, Reported_Ccell_µM, color = Name, label = round(rmsle_ccell_default, 2)))+
#   geom_point(size = 4) + 
#   geom_abline(intercept=0,slope=1, linetype = "dashed") +
#   geom_text_repel() +
#                         scale_y_continuous(labels = function(x) sprintf("%g", x),  limits = c(10, 10000), trans='log10') +
...
#                         ggtitle("Default Physchem Input")+
#                         annotate("text", x = Inf, y = Inf, label = paste("RMSLE: ", RMSLE_default.dt),
#                                  vjust = 29, hjust = 1, size = 3) +
#                         theme_bw() + 
#                         theme(legend.key.size = unit(2, 'mm'),
#                           text=element_text(size=10),
...
#                           axis.line = element_line(colour = "black")) 
# 
# curatedvalues_httk_plot<-ggplot(armitageOutput_curated.data.dt, aes(ccells, Reported_Ccell_µM, color = Name, label = round(rmsle_ccell_curated, 2)))+
#   geom_point(size = 4) + 
#   geom_abline(intercept=0,slope=1, linetype = "dashed") +
#   geom_text_repel() +
#                         scale_y_continuous(labels = function(x) sprintf("%g", x),  limits = c(10, 10000), trans='log10') +
...
#                         ylab("Experimental Intracellular Concentration (\U00B5M)")+
#                         labs(color = "Chemical Name")+
#                         ggtitle("Curated Physchem Input")+
#                         annotate("text", x = Inf, y = Inf, label = paste("RMSLE: ", RMSLE_curated.data.dt),
#                                  vjust = 29, hjust = 1, size = 3) +
#                         theme_bw() + 
#                         theme(legend.key.size = unit(2, 'mm'),
#                           text=element_text(size=10),
...
#                           axis.line = element_line(colour = "black")) 
# 
# curated_comboplot<-ggarrange(basicvalues_httk_plot, curatedvalues_httk_plot, ncol=2, legend = "bottom", common.legend = TRUE)

if(save_output){
  ggsave("Figure_5.png", path = "C:/Users/mscherer/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/ManuscriptDataJourney/ManuscriptFigures_19MAY25/",
...
max(combodata$RMSLE_nominal_by_chem) #3.553827 (Chlorpromazine) - cell conc: 338757.6, nomconc: 20.7
...
exp_nom.dt<-copy(IVD_Manuscript_data[NominalDose_uM != ExperimentalDose_uM,]) #25 entries
#unique(exp_nom.dt$Citation) #5 citations
...
final_output %>% dplyr::filter(Citation == "Tox21") %>%  select(ChemicalName, median_ratio, Citation, number_of_obs)

data.dt2%>% dplyr::filter(Citation == "Tox21") %>% select(ChemicalName, cellconc_cnom_ratio, median_ratio)
```
...
#save the output
if(save_output){
  write.csv(table_s2, path = "C:/Users/mscherer/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/ManuscriptDataJourney/ManuscriptFigures_19MAY25/table_s2_raw.csv")
}
...
#save the output
if(save_output){
  write.csv(mult.dt, path = "C:/Users/mscherer/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/ManuscriptDataJourney/ManuscriptFigures_19MAY25/table_s3.csv")
}
...
#save the output
if(save_output){
  write.csv(ionization_rmsle, path = "C:/Users/mscherer/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/ManuscriptDataJourney/ManuscriptFigures_19MAY25/table_s4.csv")
}
...
#save the output
if(save_output){
  write.csv(gkow_output, path = "C:/Users/mscherer/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/ManuscriptDataJourney/ManuscriptFigures_19MAY25/table_s5.csv")
}
...
#save the output
if(save_output){
  write.csv(kaw_output, path = "C:/Users/mscherer/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/ManuscriptDataJourney/ManuscriptFigures_19MAY25/table_s6.csv")
}
...
#147 out of the original 174 observations
...
combodata[, RMSLE_armitage:= rmsle(CellConcentration_uM.x, ccells), by = "X"]
combodata[, RMSLE_kramer:= rmsle(CellConcentration_uM.x, concentration_cells), by = "X"]
combodata[, RMSLE_nominal:= rmsle(CellConcentration_uM.x, nomconc), by = "X"]
...
if(save_output){
  ggsave("Figure_S1.png", path = "C:/Users/mscherer/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/ManuscriptDataJourney/ManuscriptFigures_19MAY25/",
...
```
#other option:
## Figure S1. Histogram showing chemical-specific RMSLE for Armitage Ccell vs Measured Ccell and Kramer Ccell vs Measured Ccell: BY CHEMICAL
```{r intracellular_histogram_plot_2, eval = execute.vignette}
...
if(save_output){
  ggsave("Figure_S1_by_chem.png", path = "C:/Users/mscherer/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/ManuscriptDataJourney/ManuscriptFigures_19MAY25/",
