---
title: "Meade et al. (2025): High Throughput Dermal Exposure Model"
author: "Annabel Meade, Celia Schacht, Alex George"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
 %\VignetteIndexEntry{i) Meade et al. (Submitted): High Throughput Dermal Exposure Model}
...
* R_CvT_Database/CvT_all_tables_2202-09-14_noSQL.Rdata: CvT data outside the [CvTdb](https://doi.org/10.1038%2Fs41597-020-0455-1) which had to be currated and loaded maually.
...
* *`r paste0("meade2023_",format(Sys.time(), "%b_%d_%Y"),".Rdata")`*: 
  + *concentration_data_meade2023*: conventration vs time and dosing data
...
![Entity-relationship diagram of CvT Database. This is an updated version of Figure 2 in [Sayre, et al. 2020](https://doi.org/10.1038%2Fs41597-020-0455-1).](R_CvT_Database/cvt_eer_diagram_20220811.png){width=500}
...
```{r load_data, eval=!(run.simulations)}
...
```{r 1_plot3, eval=TRUE, fig.height=10, fig.width=20}
...
```{r 1_exhalation_rmsle_plot,  eval=TRUE, fig.height=10, fig.width=30}
...
```{r cvt_plot_by_exh, eval =TRUE, fig.height=3, fig.width=3 }
...
```{r 1_plot1,eval =TRUE, fig.height=3, fig.width=3}
...
```{r 1_plot6, eval=TRUE, fig.height=10, fig.width=20}
...
```{r 1_exhalation_correct, eval=TRUE}
...
```{r 1_heatmap, eval=TRUE, fig.height=8, fig.width=11}
...
First we load [ToxCast Data](https://www.epa.gov/chemical-research/exploring-toxcast-data) data from the [EPA-ORD-CCTE Clowder](https://clowder.edap-cluster.com/spaces/62bb560ee4b07abf29f88fef). Some of the variables in this dataset of importance include:
...
```{r add_water_solubility, eval=run.simulations2}
...
```{r 2_load_without_running, eval = !run.simulations.2,echo=F}
...
```{r 2_plot1,fig.width=20,fig.height=12, eval=TRUE}
# Plot Histogram
...
```{r 2_dataprep_for_plot2, fig.width=15, fig.height=10,eval=TRUE}
...
```{r 2_plot2, fig.width=15, fig.height=10,eval=TRUE}
