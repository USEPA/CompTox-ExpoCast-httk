...
for(this.cas in cas.list){
  ...
  pcs <- predict_partitioning_schmitt(parameters=parameters,species='rat',regression=FALSE)
  init.pcs <- predict_partitioning_schmitt(parameters=init.parameters,species='rat',regression=FALSE)
...
fup.change.plot <-  ggplot() +
    geom_point(data=pc.table[order(pc.table[,'fup.improvement'],decreasing=FALSE),],
...
ma.change.plot <- ggplot() +
    geom_point(data=ma.subset[order(ma.subset[,'ma.improvement'],decreasing=FALSE),]
...
for(this.cas in obach[,'CAS']){
  ...
  pcs <- predict_partitioning_schmitt(parameters=parameters,regression=FALSE)
  init.pcs <- predict_partitioning_schmitt(parameters=init.parameters,regression=FALSE)
  reg.pcs <- predict_partitioning_schmitt(parameters=parameters,regression=TRUE)
...
adjustment.plot <- ggplot() +
    geom_point(data=vd.table[order(vd.table[,'adjustment.improvement'],decreasing=FALSE),],
...
calibration.plot <- ggplot() +
    geom_point(data=vd.table[order(vd.table[,'calibration.improvement'],decreasing=FALSE),],
