...
```{r convert_to_ppmweight, eval = execute.vignette}
...
```{r normalize_liquid_units, eval = execute.vignette}
...
```{r Standardize Units, eval = execute.vignette}
these.chems <- unique(subset(simobsfull,unit=="ppmv")$chem)
for (this.chem in these.chems)
{
  these.matrices <- unique(subset(simobsfull,unit=="ppmv" & chem==this.chem)$matrix)
  for (this.matrix in these.matrices)
    {
      ...
      this.factor <- convert_units(
        ...
        output.units="um", 
        chem.name=this.chem,
        state=ifelse(this.matrix %in% gas.media,"gas","liquid"))
      
      # Scale the observation    
      simobsfull[simobsfull$chem==this.chem & 
                   simobsfull$unit=="ppmv" &
                   simobsfull$matrix==this.matrix, "obsconc"] <-
      ...
      simobsfull[simobsfull$chem==this.chem & 
                   simobsfull$unit=="ppmv" &
                   simobsfull$matrix==this.matrix, "simconc"] <-
        this.factor * simobsfull[
          simobsfull$chem==this.chem & 
            simobsfull$unit=="ppmv" &
            simobsfull$matrix==this.matrix,"simconc"]
