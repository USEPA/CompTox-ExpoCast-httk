...
vignette: >
 %\VignetteIndexEntry{g) Linakis (2020): High Throughput Inhalation Model}
...
# Too many "time = 0" points already have chemical in the body:
conc_data <- subset(conc_data, TIME>0)
```


# Handle percent dose data:
```{r percent_dose, eval=FALSE}
conc_data[toupper(conc_data$ORIG_CONC_U) %in% "PERCENT","CONCENTRATION"] <- 
  conc_data[toupper(conc_data$ORIG_CONC_U) %in% "PERCENT","CONCENTRATIION"] * 
  conc_data[toupper(conc_data$ORIG_CONC_U) %in% "PERCENT","DOSE"] / 100
conc_data[toupper(conc_data$ORIG_CONC_U) %in% "PERCENT","ORIG_CONC_U"] <- 
  conc_data[toupper(conc_data$ORIG_CONC_U) %in% "PERCENT","DOSE_U"]

conc_data$ORIG_CONC <- conc_data$CONCENTRATION
conc_data$CONC_U <- conc_data$ORIG_CONC_U
```
# paper gives different doses
```{r correct_doses}
conc_data[conc_data$SOURCE_CVT=="1439659" & round(conc_data$DOSE)==2,"DOSE"] <-
  100 
conc_data[conc_data$SOURCE_CVT=="1439659" & round(conc_data$DOSE)==9,"DOSE"] <-
  300 
```
...
Although solve_gas_pbtk can now output gaseous media concentrations in both
ppmv and uM, at the time of the Linkais et al. (2020) analysis the model only
supported uM.

If the cvt data weren't already in uM we could use the following to convert:
```{r convert_to_um, eval = FALSE}
gas.media <- c("EB","MEB","EEB","EB (+W)")
target.unit <- "uM"
for (this.chem in unique(conc_data$DTXSID))
    for (this.medium in unique(subset(conc_data,
                                      DTXSID==this.chem)$SAMPLING_MATRIX))
    {
   #   this.unit <- ifelse(this.medium %in% gas.media,"ppmv","mg/l")
      this.unit <- "uM"
      this.factor <- convert_units(
        input.units=this.unit, 
        output.units=target.unit, 
        dtxsid=this.chem,
        state=ifelse(this.medium %in% gas.media,"gas","liquid"))
      print(paste("Use",
                  this.factor,
                  "to convert",
                  this.unit,
      ...
      conc_data[conc_data$DTXSID==this.chem &
                conc_data$SAMPLING_MATRIX %in% this.medium, "CONCENTRATION"] <-
        this.factor * conc_data[
          conc_data$DTXSID==this.chem &
          conc_data$SAMPLING_MATRIX %in% this.medium, "CONCENTRATION"]
      # Change the reported unit
      conc_data[conc_data$DTXSID==this.chem &
                conc_data$SAMPLING_MATRIX %in% this.medium,"CONC_U"] <-
        target.unit
    }
...
```{r summary, eval = execute.vignette}
...
### Exposure scenarios
```{r scenarios, eval = execute.vignette}
...
```{r runmodel, eval = execute.vignette}
simlist <- list()
obslist <- list()
...
# Make sure we get conc's at the observed times:
    ...
    # Set a lower measurable value:
    solve$simconc[solve$simconc<10^-6] <- 0
    simlist[[i]] <- solve
}
```
...
```{r Makestudyplots, eval = execute.vignette}
cvtlist <- list()
for(i in 1:nrow(unique_scenarios))
{
    plot.data <- simlist[[i]]
...
```{r init_dataset, eval = execute.vignette}
...
```{r merge_datasets, eval = execute.vignette}
...
observed time
```{r time_quartile, eval = execute.vignette}
...
```{r calc_AUC, eval = execute.vignette}
for(i in 1:length(simobslist))
  if (nrow(simobslist[[i]])>0)
  {
    simobscomb <- simobslist[[i]]
...
```{r calc_error, eval = execute.vignette}
for(i in 1:length(simobslist))
  if (nrow(simobslist[[i]])>0)
  {
    simobscomb <- simobslist[[i]]
...
```{r combine_studies, eval = execute.vignette}
obsvpredlist <- list()
for(i in 1:length(simobslist))
  if (nrow(simobslist[[i]])>0)
  {
...
```{r obsvspredplots, eval = execute.vignette}
...
```{r annotate_study_stats, eval = execute.vignette}
...
```{r cvtplots, eval = execute.vignette}
...
```{r combine_obs_vs_pred, eval = execute.vignette}
...
```{r regression, eval = execute.vignette}
...
```{r obspredFig2, eval = execute.vignette}
fig2 <- ggplot(
  data = simobsfull[
    simobsfull$simconc > 0 & 
    simobsfull$obsconc > 0,], 
...
```{r write_figure2, eval = execute.vignette}
...
```{r calculations, eval = execute.vignette}
...
```{r Fig4plot, eval = execute.vignette}
...
```{r write_figure4, eval = execute.vignette}
...
```{r Fig3plot, eval = execute.vignette}
...
```{r write_figure3, eval = execute.vignette}
...
```{r write_figures1, eval = execute.vignette}
...
```{r SupTable2, eval = execute.vignette}
senschem <- list()
sensslope <- list()
sensrsq <- list()
sensregrmse <- list()
...
```{r SupTable1, eval = execute.vignette}
