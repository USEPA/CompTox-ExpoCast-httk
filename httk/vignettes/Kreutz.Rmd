...
By default ggplot2 uses dashes rather than true minus signs in axis labels, this
code (from https://github.com/r-lib/scales/issues/249) corrects that:
```{r true_minus_signs, eval=TRUE}
library(scales)
library(rlang)

# Copy some internals for reproducibility
precision <- scales:::precision
round_any <- scales:::round_any

# Signs is a length 3 vector with symbols for negative, positive and zero
# signed numbers, in that order.
number <- function(x, accuracy = NULL, scale = 1, prefix = "",
  ...
  # Determine sign when x is still numeric to index with later
  x_sign <- sign(scale * x)
  x_sign[is.na(x_sign)] <- 0 # Protect against NA-indexing
  if (length(signs) < 3) {
    signs <- c(signs, rep("", 3 - length(signs)))
  }
  ...
  # Substitute existing hyphens
  ret <- gsub("-", signs[1], ret)
  # Prefix zero and plus signs
  ret[x_sign == 1] <- paste0(signs[2], ret[x_sign == 1])
  ret[x_sign == 0] <- paste0(signs[3], ret[x_sign == 0])
...
fig.wsol  <- ggplot(data=kreutz) +
  ...
  scale_x_continuous(labels = ~number(.x)) +
  theme_bw()  +
  theme( text  = element_text(size=FONT.SIZE))
...
fig.henry  <- ggplot(data=kreutz) +
  ...
  #ylab("Fraction unbound in plasma") + 
  ylab("") +
  xlab(bquote("Henry's Law Constant (Log atmospheres"%*%m^3/~"mole)")) +
  scale_x_continuous(labels = ~number(.x)) +
  theme_bw()  +
  theme( text  = element_text(size=FONT.SIZE-3))
...
fig.legend <- ggplot(data=kreutz) +
  ...
  theme(
   ...
   theme(legend.key.height= unit(0.4*1.5, 'cm'),
        legend.key.width= unit(0.4*6, 'cm'))+
  theme( text  = element_text(size=FONT.SIZE)) 
...
```
```{r fig_combined_highresfile, eval = TRUE}
png(height=1000,width=1200,res=300,file="Fig3TrendAnalysis.png",pointsize=10)
multiplot(fig.mw,
          fig.logp,fig.logd,
          fig.wsol,
          fig.henry,
