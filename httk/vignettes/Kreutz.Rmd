...
fig.logp  <- ggplot(data=kreutz) +
  ...
  geom_line(aes(x=logP,y=logPfit2),color="blue") +
...
fig.logd  <- ggplot(data=kreutz) +
  ...
  geom_line(aes(x=logD74,y=logDfit2),color="blue") +
  ylab("Fraction unbound in plasma") + 
  xlab("Distribution Coefficient in Plasma (LogD74)") +
  theme_bw()  +
  theme( text  = element_text(size=FONT.SIZE))
...
fig.wsol  <- ggplot(data=kreutz) +
  ...
  geom_line(aes(x=logWSol,y=logWSolfit2),color="blue") + 
  #ylab("Fraction unbound in plasma") + 
  ylab("") +
...
multiplot(fig.logp,fig.logd,fig.mw,fig.wsol,fig.henry,fig.legend,cols=2,widths=c(1.75,1.75))
```
