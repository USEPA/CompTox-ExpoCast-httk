...
This vignette was created for the 2017 paper with **httk** v1.7. However, it was 
...
model. We can add a new species (for example, ferrets) to the tissue data by 
replicating the data from another species and adjusting as appropriate.

```{r add_a_species, eval = TRUE}
...
knitr::kable(physiology.data[,c("Parameter","Units","Ferret")], 
             caption = "Approximate Ferret Physiological Parameters", 
             floating.environment="sidewaystable",
             row.names=FALSE,
             digits=3)
```
