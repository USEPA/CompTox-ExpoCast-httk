...
fig.clint.fup  <- ggplot(data=clint.fup) +
  geom_point(aes(
    ...
    size=POINT.SIZE) +
  ylab("In Vitro Measured Clint (uL/min/106 hepatocytes)") + 
  xlab("In Vitro Measured Fup") +
...
fig.clint.hist <- ggplot(data=clint.fup) +
  geom_histogram(aes(
    ...
    colour=Chemical.Class,
    fill=Chemical.Class)) +
  xlab("In Vitro Measured Clint (uL/min/106 hepatocytes)") +
  scale_x_log10(label=scientific_10)+
  theme_bw()  +
  theme( text  = element_text(size=FONT.SIZE))+
        scale_fill_viridis(discrete=2)+
