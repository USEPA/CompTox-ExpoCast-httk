...
vignette: >
  %\VignetteIndexEntry{X) Scherer (2025) In Vitro Distribution}
...
```{r rmsle_functions, eval = execute.vignette}
...
```{r load_IVD_Manuscript_data, eval = FALSE}
...
```{r load_IVD_Manuscript_data_from_httk, eval = execute.vignette}
# Get a working copy from HTTK:
...
# unique(IVD_Manuscript_data$Citation)
# 
# IVD_Manuscript_data %>% 
#   dplyr::filter(Citation == "Bellwon, 2015a" |
#                 Citation == "Bellwon, 2015b") %>% 
#   select(CellConcentration_uM)
...
```{r add_assay_names, eval=execute.vignette}
...
#convert from scientific format for axis labels
...
Aplastic_comparison<-ggplot(combodata, aes(concentration_plastic, cplastic, 
                        ...
                        xlab(expression("Kramer Predicted "*italic("C"["plastic"])~"(\U00B5mol/m\U00B2)")) + 
                        ylab(expression("Armitage Predicted "*italic("C"["plastic"])~"(\U00B5mol/m\U00B2)"))+
                        labs(colour = "Chemical")+
                        theme_bw() + 
...
#load data
original_curated.data.df<-read.csv("Dimitrijevic_from_arnot.csv")
```

```{r format_data, eval = execute.vignette}
original_curated.data.df <- httk::Dimitrijevic.IVD
...
#individual
armitageOutput_default.dt[,rmsle_ccell_default:=rmsle(Reported_Ccell_uM, ccells), by = Name]

#total
RMSLE_default.dt<-rmsle(armitageOutput_default.dt$Reported_Ccell_uM, armitageOutput_default.dt$ccells)
...
#individual
armitageOutput_curated.data.dt[,rmsle_ccell_curated:=rmsle(Reported_Ccell_uM, ccells), by = Name]

#total
RMSLE_curated.data.dt<-rmsle(armitageOutput_curated.data.dt$Reported_Ccell_uM, armitageOutput_curated.data.dt$ccells)
...
```{r make_physchem_figure, eval = execute.vignette}
curated_difference_httk_plot<-ggplot(curated_data.dt, aes(ccells.y, Reported_Ccell_uM.y, color = Name, label = round(rmsle_difference, 2)))+
  geom_point(size = 4) + 
  geom_abline(intercept=0,slope=1, linetype = "dashed") +
  geom_text_repel() +
...
```{r calculate_accuracy_change, eval = execute.vignette}
...
```{r physchem_props, eval = execute.vignette}
...
```{r options_plot, eval = execute.vignette}
