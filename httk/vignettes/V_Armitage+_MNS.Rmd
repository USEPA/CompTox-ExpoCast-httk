...
INSERT FIGSHARE LINK TO POSTER HERE
...
devtools::load_all("httk") # for armitage
```
...
IVD_data <- read_csv("ExtractedData_27FEB2025.csv")
...
##check ivd_data is a data.table (otherwise convert...)
...
named_poster_pred<-ggplot(armitageOutput_data.dt, aes(ccells, CellConcentration_uM,
                        ...
                        ylab("Experimental Intracellular Concentration (\U00B5M)")+
                        annotate(geom="text", x=300, y=0.001, label="RMSLE: 2.13") +
                        theme_bw() + #now make it look good
                        theme(legend.position="bottom",
                          panel.border = element_blank(), 
                              panel.grid.major = element_blank(),
...
# Table for Nominal and Modeled Values by Ionization State
