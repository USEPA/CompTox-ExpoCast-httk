...
### Figure 1: 
Histograms depicting the median measured values (a, b) and 95% credible intervals (c, d) for fraction unbound in plasma (fup, a,c) and intrinsic hepatic clearance (Clint, b,d). In panel a, the distribution of estimated fup for the single protein concentration assay (solid line) is compared with the protein titration (three concentrations) method (dotted) line. In panel b, the median estimated Clint is shown. In panel c, the distribution with larger credible intervals corresponds to the single protein concentration assay, with a median credible interval is 0.1, corresponding to a precision of roughly Ãƒâ€šÃ‚Â±0.05. When data from the protein titration are jointly analyzed (smaller credible intervals), the certainty is increased to a median credible interval of 0.029, or roughly Ãƒâ€šÃ‚Â±0.015. The width of the credible interval is a measure of the certainty in an estimate, i.e., a smaller credible interval indicates greater certainty. For the fup values, a credible interval approaching 1 indicates that all possible values are consistent with the data ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ that is, the data do not help identify fup since fup must be between 0 and 1. In panel d the distribution of credible intervals for Clint is shown.
```{R Wambaugh2019.Fig1}
...
Fig1b <- ggplot(Fig1.table)+
   ...
   xlab(expression(paste("Measured ",Cl[int]," (Ãƒâ€šÃ‚ÂµL/min/",10^6," hepatocytes)"))) +
  ylab("Number of chemicals")+
...
#Figure 4:
Comparison of Bayesian Clint and Uncertainty Estimations to Experimental Point Estimates. Clint estimate for 1 Ãƒâ€šÃ‚ÂµM chemical concentration displayed, though 1 and 10 Ãƒâ€šÃ‚ÂµM were fit jointly. The size of the credible interval varies significantly from chemical to chemical, especially for the lower clearance rates. Plots of individual fits are provided as supplemental material 
...
Fig5b <- ggplot(data=ceetox,aes(x=cssmed,y=cssmed_u)) +
  ...
  xlab(expression(paste(C[ss]," Point Estimate (Ãƒâ€šÃ‚ÂµM)")))+
  ylab(expression(paste("Bayesian ",C[ss]," (Ãƒâ€šÃ‚ÂµM)")))+
...
#Figure 6:
Relative contributions of uncertainty and variability to differences between the 95th percentile and median Css. Monte Carlo analysis of both variability (triangles) and uncertainty (circles) give distributions that can be characterized by a 95th percentile Css for which individuals achieve a higher Css for the same fixed dose rate ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ these individuals can be considered more ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œsensitiveÃƒÂ¢Ã¢â€šÂ¬Ã‚Â to chemical exposure. Uncertainty and variability can be combined (squares) to estimate a 95th percentile reflecting both factors. For most chemicals, variability contributes more than uncertainty to the difference between the Css predicted for the median Clint and fup values. 
...
# This has been Subset  to just the chemicals we have new numbers for:
#Tox21.acc <- httk::wambaugh2019.Tox21
...
# In this vignette we use the AC50's (as in Ring 2017) rather than ACC's
# (as in Wambaugh 2019) to help keep the HTTK package smaller:
Tox21.acc.numeric <- httk::ToxCast2015subset[,c("Chemical.Name",
  "CASRN",
  "Assay.Endpoint",
  "AC50",
  "Activity.Call")]
colnames(Tox21.acc.numeric) <- c("chnm","casn","variable","value","call")
...
{
  med.conc <- median(10^(subset(Tox21.acc.numeric,casn==this.chem)$value))
  ...
  q90.conc <- quantile(10^(subset(Tox21.acc.numeric,casn==this.chem)$value),0.9)
...
median(Wetmore.chems$Css.v110/Wetmore.chems$Css.Lit,na.rm=T)
...
write.csv(ceetox[,c(1,2,8,9,16,17,6,7,12,30,10,11,15,31,13,14,18:28)],row.names=F,file=paste("Supplemental-Table4-",Sys.Date(),".txt",sep=""))
