...
for(i in 1:length(simlist))
{
  ...
  { #skips over anything for which there was no observed data or insufficient information to run simulation
    ...
    simobscomb$chemclass <- met_data$CHEM_CLASS[unique(met_data$PREFERRED_NAME) %in%
      unique(simobscomb$chem)][1]
    simobscomb$logp <- met_data$OCTANOL_WATER_PARTITION_LOGP_OPERA_PRED[
      unique(met_data$PREFERRED_NAME) %in% unique(simobscomb$chem)][1]
    simobscomb$sol <- met_data$WATER_SOLUBILITY_MOL.L_OPERA_PRED[
      unique(met_data$PREFERRED_NAME) %in% unique(simobscomb$chem)][1]
    simobscomb$henry <- met_data$HENRYS_LAW_ATM.M3.MOLE_OPERA_PRED[
      unique(met_data$PREFERRED_NAME) %in% unique(simobscomb$chem)][1]
    simobscomb$orig_conc_u <- obsdata$ORIG_CONC_U
    simobscomb$vmax <- met_data$VMAX[unique(met_data$PREFERRED_NAME) %in%
      unique(simobscomb$chem)][1]
    simobscomb$km <- met_data$KM[unique(met_data$PREFERRED_NAME) %in%
      unique(simobscomb$chem)][1]
    if (nrow(simobscomb)>0) 
      for (j in 1:nrow(simobscomb))
    {
      ...
      if (simobscomb$time[j]/max(simobscomb$time) >= 0 & 
          simobscomb$time[j]/max(simobscomb$time) < 0.25)
      {
        simobscomb$tquart[j] <- "1"
      } else if(simobscomb$time[j]/max(simobscomb$time) >= 0.25 & 
                simobscomb$time[j]/max(simobscomb$time) < 0.5) {
        simobscomb$tquart[j] <- "2"
      } else if(simobscomb$time[j]/max(simobscomb$time) >= 0.5 & 
                simobscomb$time[j]/max(simobscomb$time) < 0.75) {
        simobscomb$tquart[j] <- "3"
      } else if(simobscomb$time[j]/max(simobscomb$time) >= 0.75 & 
                simobscomb$time[j]/max(simobscomb$time) <= 1){
