...
# If multiple observations are mapped onto the same simulated time,
# then duplicate that row of the simulation:    
    simobscomb <- rbind(simobscomb,subset(simobscomb,time %in%
      signif(obsdata$TIME,4)[duplicated(signif(obsdata$TIME,4))]))
# Get everything in order chronologically:
    simobscomb <- simobscomb[order(simobscomb$time),]
# Merge with obsdata (why not use merge?)
