...
for(i in 1:length(simobslist))
  if (nrow(simobslist[[i]])>0)
  {
    simobscomb <- simobslist[[i]]
    for (j in 1:nrow(simobscomb))
      if (!is.na(simobscomb$time[j])) 
    {
      if (simobscomb$time[j]/max(simobscomb$time,na.rm=T) >= 0 & 
          simobscomb$time[j]/max(simobscomb$time,na.rm=T) < 0.25)
      {
        simobscomb$tquart[j] <- "1"
      } else if(simobscomb$time[j]/max(simobscomb$time,na.rm=T) >= 0.25 & 
                simobscomb$time[j]/max(simobscomb$time,na.rm=T) < 0.5) {
        simobscomb$tquart[j] <- "2"
      } else if(simobscomb$time[j]/max(simobscomb$time,na.rm=T) >= 0.5 & 
                simobscomb$time[j]/max(simobscomb$time,na.rm=T) < 0.75) {
        simobscomb$tquart[j] <- "3"
      } else if(simobscomb$time[j]/max(simobscomb$time,na.rm=T) >= 0.75 & 
                simobscomb$time[j]/max(simobscomb$time,na.rm=T) <= 1){
