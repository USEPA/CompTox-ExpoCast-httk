...
vignette: >
  %\VignetteIndexEntry{Ring et al. 2017 Global sensitivity analysis plotting}
...
library(scales)
```
First, read in the global sensitivity analysis data. Do this for all four combinations of the `poormetab` and `fup.censored.dist` conditions.
...
dat <- merge(dat,
  ...
  dat[, param:=factor(param, 
                              levels=c('CLint',
                                       ...
                                       'phys.par'))]
```
Next, set up a few things to prepare for plotting. First, change the factor levels for `poormetab` and `fup.censored.dist` to be more informative than just TRUE and FALSE.
