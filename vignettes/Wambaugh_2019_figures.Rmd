...
# Wambaugh et al. (2019): Creating All Figures
...
### Figure 1: 
Histograms depicting the median measured values (a, b) and 95% credible intervals (c, d) for fraction unbound in plasma (fup, a,c) and intrinsic hepatic clearance (Clint, b,d). In panel a, the distribution of estimated fup for the single protein concentration assay (solid line) is compared with the protein titration (three concentrations) method (dotted) line. In panel b, the median estimated Clint is shown. In panel c, the distribution with larger credible intervals corresponds to the single protein concentration assay, with a median credible interval is 0.1, corresponding to a precision of roughly Ã‚Â±0.05. When data from the protein titration are jointly analyzed (smaller credible intervals), the certainty is increased to a median credible interval of 0.029, or roughly Ã‚Â±0.015. The width of the credible interval is a measure of the certainty in an estimate, i.e., a smaller credible interval indicates greater certainty. For the fup values, a credible interval approaching 1 indicates that all possible values are consistent with the data Ã¢â‚¬â€œ that is, the data do not help identify fup since fup must be between 0 and 1. In panel d the distribution of credible intervals for Clint is shown.
```{R Wambaugh2019.Fig1}
...
Fig1b <- ggplot(Fig1.table)+
   ...
   xlab(expression(paste("Measured ",Cl[int]," (Ã‚ÂµL/min/",10^6," hepatocytes)"))) +
  ylab("Number of chemicals")+
...
#Figure 4:
Comparison of Bayesian Clint and Uncertainty Estimations to Experimental Point Estimates. Clint estimate for 1 Ã‚ÂµM chemical concentration displayed, though 1 and 10 Ã‚ÂµM were fit jointly. The size of the credible interval varies significantly from chemical to chemical, especially for the lower clearance rates. Plots of individual fits are provided as supplemental material 
...
Fig5b <- ggplot(data=ceetox,aes(x=cssmed,y=cssmed_u)) +
  ...
  xlab(expression(paste(C[ss]," Point Estimate (Ã‚ÂµM)")))+
  ylab(expression(paste("Bayesian ",C[ss]," (Ã‚ÂµM)")))+
